...
namespace Dissonance.Integrations.Unity_NFGO
{
        ...
        >
    {
            ...
            if (IsInitialized)
            {
                ...
                if (Server != null)
                {
                    foreach (var item in _loopbackQueueToServer)
                    {
                        if (item.Array != null)
                        {
                            Server.NetworkReceivedPacket(new NfgoConn(null), item);
                            _loopbackBuffers.Put(item.Array);
                        }
                    }
                }
        ...
        internal void SendToClient(ArraySegment<byte> packet, NfgoConn client, bool reliable, [NotNull] NetworkManager netManager)
        {
            ...
            else
            {
                ...
                if (!reliable && !netManager.ConnectedClients.ContainsKey(client.ClientId.Value))
                    return;
    ...
    {
        /// <summary>
        /// ClientID of this connection, null indicates loopback
        /// </summary>
